...
本文档中的**语言层面规范**——包括报道条目的句式模板及动词完成体标记、高频中心动词清单、
时间状语与编辑用语的写法、条目的字数与句数等量化指标——均以**简体中文版《每周一龙》**
的文本为归纳与分析对象（参见[计量分析脚本](#analysis-tooling)）。
这些规范只适用于中文内容的撰写与审校；目前 TWiL 仅以简体中文发布。

本文档不为其他语言版本的 TWiL（例如未来可能出现的英文版）规定语言层面的规范，
此类规范留待相应语言版本的文体指南另行制定。本文档中与语言无关的规范
（如报道准则、科普尺度等）不受此影响，对所有语言版本均适用。
...
绝大多数条目只有 **一到两句话**。如果一个条目超过了 3 句话，应当考虑拆分（前提是不影响可读性），或确认其确实构成一个不可分割的报道单元。
...
| **动词选择** | `添加了` | 优先替换为更具体的动词（`实现了`、`启用了`、`引入了` 等）；仅在无合适替代时使用 `增加了`。参见[动词的具体性](#verb-specificity)。 |
...
5. **统一措辞**：检查并替换偏离风格指南的关键词汇。尤其是将抽象的 `添加`/`增加` 替换为更具体的动词（如 `实现了`、`启用了`、`引入了`）；仅在无合适替代时保留 `增加`。参见[动词的具体性](#verb-specificity)。
